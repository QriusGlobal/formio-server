...
module.exports = function(options) {
  ...
  // Mount the portal application.
  if (fs.existsSync(path.join(__dirname, '/portal/dist'))) {
    app.use('/portal', express.static(path.join(__dirname, '/portal/dist')));
    // Redirect root to portal
    app.get('/', (req, res) => {
      res.redirect('/portal');
    });
    util.log(' > Portal available at /portal'.green);
  }
  else {
    // Fallback to client if portal doesn't exist
  ...
  const securityHeaders = require('./src/middleware/securityHeaders')(server);
  app.use(securityHeaders);

  // Apply rate limiting middleware
  const rateLimiting = require('./src/middleware/rateLimiting')(server);
  ...
  server.init(hooks).then(function(formio) {
    ...
    const start = function() {
      ...
      // Mount the Form.io API platform at root
      // Portal expects API at root (compiled with this assumption)
      const apiMount = options.mount || '/';
      app.use(apiMount, server);
      util.log(` > API mounted at ${apiMount.green}`);
