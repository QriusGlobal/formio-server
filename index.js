...
module.exports = function(config) {
  ...
  router.init = async function(hooks) {
    ...
    // ============================================================================
    // SYSTEM HEALTH ROUTES - MUST BE REGISTERED BEFORE ALIAS MIDDLEWARE
    // ============================================================================
    // These routes bypass Form.io's alias lookup to prevent "Invalid alias" errors
    // when infrastructure tools (Docker, Kubernetes, load balancers) query health endpoints.
    //
    // CRITICAL: These MUST come before setupMiddlewares() which registers the
    // wildcard alias middleware that would otherwise intercept these paths.
    // ============================================================================

    // Liveness probe - indicates application is running
    ...
    // Readiness probe - indicates application can handle traffic
    router.get('/ready', async (req, res) => {
      try {
        const checks = {};

        // Check MongoDB connection
        const dbState = router.formio.mongoose.connection.readyState;
        checks.database = dbState === 1 ? 'connected' : 'disconnected';

        // Check Redis (if configured)
        if (router.formio.redis) {
          try {
            await router.formio.redis.ping();
        ...
        const allReady = checks.database === 'connected' &&
                         (checks.cache === 'connected' || checks.cache === 'not configured');
    ...
    // ============================================================================
    // NOW SETUP MIDDLEWARE STACK (including alias middleware)
    // ============================================================================
